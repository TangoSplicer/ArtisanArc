...
class ImageService {
  static Future<List<Color>> extractDominantColors(String imagePath,
      {int colorCount = 5}) async {
    try {
      ...
      for (int y = 0; y < resized.height; y++) {
        for (int x = 0; x < resized.width; x++) {
          ...
          final colorKey = (color.r.toInt() << 16) |
              (color.g.toInt() << 8) |
              color.b.toInt();
  ...
  static Future<String> compressAndSaveImage(
      String originalPath, String targetDirectory) async {
    try {
