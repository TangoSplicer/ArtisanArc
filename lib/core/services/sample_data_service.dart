...
class SampleDataService {
  ...
  static Future<bool> hasCraftData() async {
    ...
    return inventory.isNotEmpty || sales.isNotEmpty || projects.isNotEmpty || shopping.isNotEmpty;
  }

  static Future<void> loadStarterData({bool replaceExisting = false}) async {
    if (await hasCraftData() && !replaceExisting) {
      throw StateError('Existing craft data must be cleared or replaced first.');
    }
    ...
    if (replaceExisting) {
      await Future.wait([inventory.clear(), sales.clear(), projects.clear(), shopping.clear()]);
    }
    ...
    final sampleProject = Project(
      ...
      description: 'A beginner-friendly crochet tote using cotton yarn. Sample project for learning the planner.',
      craftType: 'Crochet',
      startDate: now.subtract(const Duration(days: 3)),
      endDate: now.add(const Duration(days: 11)),
      milestones: [
        Milestone(id: 'sample-milestone-colours', name: 'Choose colour palette', description: 'Select three cotton yarn colours.', dueDate: now.add(const Duration(days: 1)), isCompleted: true),
        Milestone(id: 'sample-milestone-squares', name: 'Crochet 13 granny squares', description: 'Keep a consistent tension and block as you go.', dueDate: now.add(const Duration(days: 6))),
        Milestone(id: 'sample-milestone-assemble', name: 'Assemble, line and finish', dueDate: now.add(const Duration(days: 11))),
      ],
      supplyNeeds: [
        SupplyNeed(id: 'sample-supply-cotton', itemName: 'DK Cotton Yarn — Sage', quantityNeeded: 4, unit: 'ball', isSourced: true),
        SupplyNeed(id: 'sample-supply-hook', itemName: '4 mm Ergonomic Crochet Hook', quantityNeeded: 1, unit: 'piece', isSourced: true),
      ],
    ...
    final sampleSales = <SaleRecord>[
      SaleRecord(id: 'sample-sale-bee', itemId: 'sample-finished-bee', quantity: 3, pricePerUnit: 5.00, date: now.subtract(const Duration(hours: 3)), eventName: eventName, eventLocation: eventLocation),
      SaleRecord(id: 'sample-sale-basket', itemId: 'sample-finished-basket', quantity: 1, pricePerUnit: 18.00, date: now.subtract(const Duration(hours: 2, minutes: 20)), eventName: eventName, eventLocation: eventLocation),
      SaleRecord(id: 'sample-sale-cozy', itemId: 'sample-finished-cozy', quantity: 2, pricePerUnit: 9.00, date: now.subtract(const Duration(hours: 1, minutes: 10)), eventName: eventName, eventLocation: eventLocation),
    ];
    ...
    final sampleShoppingList = ShoppingList(
      ...
      createdAt: now,
      items: [
        ShoppingListItem(id: 'sample-shopping-cotton', itemName: 'DK cotton yarn', quantity: '4 balls', notes: 'Teal, cream and coral'),
        ShoppingListItem(id: 'sample-shopping-eyes', itemName: 'Safety eyes', quantity: '1 pack', notes: '8 mm black'),
        ShoppingListItem(id: 'sample-shopping-stuffing', itemName: 'Toy stuffing', quantity: '1 bag'),
        ShoppingListItem(id: 'sample-shopping-markers', itemName: 'Locking stitch markers', quantity: '1 pack'),
      ],
    );
